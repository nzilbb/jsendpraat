...
public class FileDownloader extends Thread {
  ...
  /**
   * Getter for {@link #bDowncaseExtensions}: Whether file extensions are to be made 
   * lowercase or not.
  ...
  /**
   * Setter for {@link #bDowncaseExtensions}: Whether file extensions are to be made 
   * lowercase or not.
   * @param bNewDowncaseExtensions Whether file extensions are to be made lowercase or not.
   */
  public void setDowncaseExtensions(boolean bNewDowncaseExtensions) {
    bDowncaseExtensions = bNewDowncaseExtensions;
  }
  ...
  public FileDownloader(
    URL url, final JProgressBar pb, IMessageHandler messageHandler, String authorization) {
    url_ = url;
    pb.setStringPainted(true);
    pb_ = new IProgressIndicator() {
        public void setMaximum(int max) { pb.setMaximum(max); }
   ...
   */
  public FileDownloader(
    URL url, IProgressIndicator pb, IMessageHandler messageHandler, String authorization) {
    url_ = url;
    pb_ = pb;
    messageHandler_ = messageHandler;
    addAuthorization(url, authorization);
   ...
   * @param authorization The Authorization HTTP header value.  If this is null or empty, 
   * no authorization is added, and the method silently returns.
   */
   ...
   * @param url
   * @return The local file containing the content of the given URL, or null if it has 
   * not been downloaded.
   */
          ...
          if (localFile_ == null || !localFile_.exists()) {
            ...
            try {
              ...
              if (strSuffix == null) {
                ...
                if (iLastSlash >= 0) {
                  strSuffix = "-" + url_.getPath().substring(iLastSlash+1);
                }
              }
              if (strSuffix == null) {
  ...
  /**
   * Opens a connection to the given URL.  If the connection requires a username/password, 
   * the user is asked to provide it.
   * @param url An HTTP URL.
   * @return A connection to the URL, or null if the user cancels out of entering a 
   * username/password.
   * @throws Exception
   */
